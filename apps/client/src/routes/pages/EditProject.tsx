...
const EditProject = () => {
  ...
  const [teamDetail, projectDetail] = useQueries({
    queries: [
      ...
      },
      {
        ...
        queryFn: async () => getProjectDetail(projectId!),
      },
    ],
  });
  ...
  useEffect(() => {
    if (projectDetail.data) {
      const member = projectDetail.data.projectMembers.filter(
        (m) => m.id !== userId,
      );
      setSelectedMember(member);
    }
  }, [projectDetail.data, userId]);

  const editProjectHookForm = useForm<ICreateProjectDto>({
    resolver: zodResolver(createProjectSchema),
    values: {
      name: projectDetail.data?.title || "",
      description: projectDetail.data?.description || "",
      profileImage: projectDetail.data?.profileImage || null,
    },
  });

  const onSubmit = (data: ICreateProjectDto) => {
    const members = selectedMember.map((member) => member.id);
    updateProjectMutation.mutate({ data, members });
  };
    ...
    <>
      <CommonContainer>
        <form
          ...
          onSubmit={editProjectHookForm.handleSubmit(onSubmit)}
        >
          <div className="flex flex-col gap-8 pt-16">
            <input
              ...
              {...editProjectHookForm.register("name")}
            />
              ...
              <InputRow title="설명" multiLine>
                <div className=" flex-1">
                  <textarea
                    ...
                    {...editProjectHookForm.register("description")}
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              <p className="typo-base typo-medium text-white">수정</p>
            </button>
          </div>
        </form>
      </CommonContainer>
